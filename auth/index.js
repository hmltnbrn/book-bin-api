...
exports.signUp = function(req, res, next) {
  return db.query("SELECT * FROM cl_sign_up($1, $2, $3, $4, $5, $6, $7, $8)", [req.body.username, req.body.password, req.body.firstName, req.body.lastName, req.body.email, req.body.zip, req.body.schoolName, req.body.role], true, req.body.username + " sign up attempt")
    .then(user => {
      if(user.cl_sign_up == 'false') return res.json({ status: false, message: "Username or email already in use" });
...
exports.activateAccount = function(req, res, next) {
  return db.query("SELECT * FROM cl_activate_account($1)", [req.body.token], true, "Activate account attempt")
    .then(user => {
      if(!user.cl_activate_account) return res.json({ status: false, message: "Token invalid" });
...
exports.forgotPassword = function(req, res, next) {
  return db.query("SELECT * FROM cl_password_token($1)", [req.body.email], true)
    .then(token => {
      if(token.cl_password_token == 'false') return res.json({ status: false, message: "Email is not in use" });
...
exports.resetPassword = function(req, res, next) {
  return db.query("SELECT * FROM cl_reset_password($1, $2, $3)", [req.body.email, req.body.token, req.body.password], true, req.body.email + " reset password attempt")
    .then(token => {
      if(!token.cl_reset_password) return res.json({ status: false, message: "Token invalid or expired" });
